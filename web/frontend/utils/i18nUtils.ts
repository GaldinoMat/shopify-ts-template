...
async function loadIntlPluralRulesLocaleData(locale: string) {
  return (
    await PLURAL_RULES_LOCALE_DATA[
      locale as keyof typeof PLURAL_RULES_LOCALE_DATA
    ]()
  ).default;
}
/**
...
function localResourcesToBackend() {
  return resourcesToBackend(async (locale: string, _namespace: any) => {
...
async function loadPolarisTranslations(locale: string) {
  return (
    await POLARIS_LOCALE_DATA[locale as keyof typeof POLARIS_LOCALE_DATA]()
  ).default;
}
